...
class ConceptofacturacionController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!isset($name)) {
            ...
            //Con paginación
            $conceptosfacturacion = Conceptofacturacion::orderBy('concepto', 'asc')->paginate(10);
            return view('conceptosfacturacion.index',compact('conceptosfacturacion'));
            //al usar esta paginacion, recordar poner en el el index.blade.php este codigo  {!! $roles->links() !!}
        } else {
            //$roles = Role::where('name', 'like', '%' .$name . '%')->orderBy('updated_at', 'DESC')->paginate(10);
            $conceptosfacturacion = Conceptofacturacion::whereRaw('UPPER(concepto) LIKE ?', ['%' . strtoupper($name) . '%'])->orderBy('concepto', 'asc')->paginate(10);
            return view('conceptosfacturacion.index',compact('conceptosfacturacion'));
        }
    }
    ...
    {
        return view('conceptosfacturacion.crear');   
    }
       ...
       // return view('iprousuarios.editar',compact('iprousuario'));
        /*
            ...
            */
            ...
            return view('conceptosfacturacion.editar',compact('concepto'));
    ...
    public function destroy($idconcepto)
    {
       // return redirect()->route('iprousuarios.index')->with('mensaje','Usuario borrado con éxito!.');
        ...
        $nombre = Conceptofacturacion::where('idconcepto',$idconcepto)->pluck('concepto')->first();

        $modelo = Conceptofacturacion::where('idconcepto',$idconcepto)->delete();

        return redirect()->route('conceptofacturacion.index')->with('mensaje','Concepto '.$nombre.' borrado con éxito!.');    
