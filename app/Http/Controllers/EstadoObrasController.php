...
class EstadoObrasController extends Controller
{
    ...
    function __construct()
    {
        ...
        $this->middleware('permission:VER-ESTADOOBRAS|CREAR-ESTADOOBRAS|EDITAR-ESTADOOBRAS|BORRAR-ESTADOOBRAS', ['only' => ['index']]);
    ...
    {      
        ...
        if ($periodo =='') {
            ...
            $estadoobras = EstadoObras::empresa($empresa)
                                        ->programa($programa)
                                        ->where('periodo',$periodoBusqueda)->orderBy('nom_emp', 'asc')->simplePaginate(10);

            $dt->formatLocalized('%B');
            $periodoMostrar = $dt->format('F\\, Y');
            return view('estadoobras.index',compact('estadoobras','periodo','periodoMostrar','empresa','programa'));
        }
    }
