...
@section('content')
    ...
    <section class="section">
        <div class="section-header">
            <h3 class="page__heading">Modificar Víctima</h3>
        </div>
            ...
            <div class="row">
                <div class="card">
                    <div class="card-body">
                        ...
                        {!! Form::model($victima,['method'=>'PATCH',
                                                'class' => 'formulario',
                                                'route'=>['rrhh.denuncias.victima.update', $victima->id_denuncia]
                                                ])!!}
                            <div class="row">
                                <div class="form-group">
                                    ...
                                    {!! Form::text('apellido_victima', $victima->apellido, [
                                        'id' => 'apellido_victima',
                                        'class' => 'form-control',
                                        'maxlength' => '50',
                                        ...
                                        'required' => true]) !!}
                                    {!! Form::text('nombres_victima', $victima->nombre, [
                                        'id' => 'nombres_victima',
                                        'class' => 'form-control',
                                        'maxlength' => '50',
                                    ...
                                    <div class="form-group" style="display: flex; align-items:center; justify-content: flex-start;margin-bottom:0%">
                                        <select name="tipo-doc" id="tipo-doc" class="form-select" style="max-width: 300px;" required>
                                            @foreach ($todosLosTipdoc as $tipdoc)
                                                <option value="{{ $tipdoc->id_tipdoc }}" {{ $tipdoc->id_tipdoc == $victima->tipo_doc ? 'selected' : '' }}>{{ $tipdoc->des_abr }} - {{ $tipdoc->destipdoc }}</option>
                                            @endforeach
                                        </select>
                                        {!! Form::number('num-doc', $victima->nro_doc, ['type' => 'number','class' => 'form-control', 'style'=> 'max-width: 300px;margin-left: 10px', 'min' => 1, 'max' => 999999999, 'placeholder' => 'INGRESE NÚMERO', 'required' => true]) !!}
                                    </div>
                                </div>
                                ...
                                <div class="form-group" style="display: flex; justify-content:flex-start; align-items:flex-start;margin:10px 0 0 0">
                                    <select name="tipo-sex" id="tipo-sex" class="form-select" style="width: 30%;margin-bottom:15px;margin-top:0px" required>
                                        @foreach ($todosLosSexo as $tipoSexo)
                                            <option value="{{ $tipoSexo->codsexo }}" {{ $tipoSexo->codsexo == $victima->id_sexo ? 'selected' : '' }}>{{ $tipoSexo->descsexo }}</option>
                                        @endforeach
                                    </select>
                                    {!! Form::date('fecha-nac',\Carbon\Carbon::parse($victima->fecha_nac),['class'=>'form-control date-field mb-3', 'style' => 'width: auto;justify-content:left;margin:0 0 0 20px', 'max' => now()->format('Y-m-d')]) !!}
                                </div>
                                <div class="form-group">
                                    {!! Form::label('direccion', 'Dirección:', ['class' => 'form-label', 'style' => 'margin-bottom:0px;']) !!}
                                    {!! Form::text('direccion', $victima->domicilio, [
                                        'id' => 'direccion',
                                        'class' => 'form-control',
                                        'maxlength' => '50',
                                ...
                                <div class="form-group">
                                    ...
                                    {!! Form::email('email', $victima->mail, ['class' => 'form-control', 'style' => 'max-width: 300px;', 'maxlength' => '50',]) !!}
                                </div>
                                <div class="form-group">
                                    {!! Form::label('tel', 'Teléfono:', ['class' => 'form-label', 'style' => 'margin-bottom:0px;']) !!}
                                    {!! Form::tel('tel', $victima->telefono, ['class' => 'form-control', 'style'=> 'max-width: 300px;', 'maxlength' => '20',]) !!}
                                </div>
                                <div class="form-group">
                                    ...
                                    <select name="tipo-vinculo" id="tipo-vinculo" class="form-select" style="max-width: 300px;">
                                        @foreach ($todosLosVinculos as $vinculo)
                                            <option value="{{ $vinculo->id_vinculo }}" {{ $vinculo->id_vinculo == $victima->vinculo_inst ? 'selected' : ''}}>{{ $vinculo->descripcion }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-xs-12 col-sm-12 col-md-12">
                                ...
                                @endcan
                                <a href="{{ route('rrhh.denuncias.intervinientes', ['id' => $victima->id_denuncia]) }}" class="btn btn-secondary mr-2">Volver</a>
                            </div>
