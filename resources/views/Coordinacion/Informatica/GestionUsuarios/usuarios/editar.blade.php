                                                ...
                                                <div id="rolles" class="d-flex flex-column overflow-auto">
                                                    ...
                                                    @foreach ($roles as $value)
                                                        @foreach ($userRoles as $rol)
                                                            @if ($value->name == $rol->name)
                                                                <label
                                                                    id="3{{ $value->id }}">{{ Form::checkbox('roles[]', $value->id, true, ['onclick' => 'buscarpermisos(' . $value->id . ')', 'class' => 'name']) }}
                                                                    {{ $value->name }}</label>
                                                                @php
                                                                    $bandera = true;
                                                                @endphp
                                            ...
                                            <div class="card-body ms-2 " id="lista_roles"style="height: 225px;width:50%">
                                                ...
                                                <div id="permisos2" class="d-flex flex-column overflow-auto"
                                                    ...
                                                    @foreach ($permisos as $value)
                                                        <label
                                                            id="2{{ $value->name }}">{{ Form::checkbox('permisos2[]', $value->id, false, ['onclick' => 'agregarpermiso(' . $value->id . ',"' . $value->name . '")', 'class' => 'name']) }}
                                                            {{ $value->name }}</label>
                                                    @endforeach
                                            ...
                                            <div class="card me-3  mt-3 " style="height: 225px; width:100% ">
                                                ...
                                                <div class="overflow-auto">
                                                    <div class="card-body d-flex flex-column pt-0" id="lista_permisos2">
                                                        ...
                                                        <label id="{{ $value->name }}"><input checked onclick="eliminar({{ $value->id }},'{{ $value->name }}')" class="name me-2" name="permisos[]" type="checkbox" value="{{ $value->id }}">{{ $value->name }}</label>
                                        ...
                                        <div class="d-flex flex-row align-items-start justify-content-around ">
                                            <div class="card-body ms-2 " id="lista_roles"style="height: 225px;width:50%">
                                                ...
                                                @foreach ($roles_sinpermisos as $value)
                                                    @foreach ($userGrupo as $grup)
                                                        @if ($value->name == $grup->name)
                                                            <label
                                                            id="4{{ $value->id }}">{{ Form::checkbox('grupos[]', $value->id, true, ['class' => 'name']) }}
                                                            {{ $value->name }}</label>
                                                            @php
                                                                $bandera = true;
                                                            @endphp
